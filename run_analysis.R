run_analysis <- function(){
  ...
  ## Build up the matrix (extracted) with only the std and mean columns, plus the correct column names (cols)  
  for(i in 1:col_cnt){
  ...
  cols<-c("subj","act",cols)
  
  ## Name the columns correctly using the built up column vector
  colnames(extracted)<-cols
  
  ## Join the activ.labels and extracted tables to get accociate the activity names with the numeric key
  res2<-merge(activ.labels,extracted)
  
  ## Remove the now superfluous act, that is number column
  res3<-select(res2,-act)
